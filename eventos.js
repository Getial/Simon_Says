...
class Juego {
    ...
    verificarNivelInicial(){
        if(this.nivelInicial >= this.niveles){
            ...
            swal('Hey', 'El nivel inical no puede ser mayor al numero de niveles seleccionados', 'error')
            .then(() => {
    ...
    iluminarSecuencia() {        
        ...
        for(let i = 0; i < this.nivel; i++) {
            ...
            setTimeout(() => this.iluminarPosicion(numeroEnString), this.tiempoDificultad * i)
        }
        setTimeout(() => this.agregarEventosClick(), (this.tiempoDificultad - 100) * this.nivel)
    }
    iluminarPosicion(numeroEnString) {
        this.numeros[numeroEnString].classList.add(this.iluminar)
        setTimeout(() => this.apagarPosicion(numeroEnString), this.tiempoDificultad - 100)
    }
    ...
    ganoElJuego() {
        swal('HEY!', `Felicitaciones ${this.name}, ganaste el juego`, 'success')
        .then(this.inicializar)
    }
    perdioElJuego() {
        swal('Ups!', `Lo lamento ${this.name}, perdiste`, 'error')
        .then(() => {
            this.eliminarEventosClick()
            this.inicializar()
        })
    ...
    definirNiveles(){
        swal('Hey', 'Debes definir el numero de niveles', 'error')
        .then(() => {
            this.eliminarEventosClick()
            this.inicializar()
        })
